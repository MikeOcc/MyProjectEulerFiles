...
for hands in x:
  ...
  # convert letter cards to numerical values    
  for i in xrange(10):
    if i%2==0:
      if hand2[i] in "TJQKA":
  ...
  #  sort hands first by determining the set of each hand with Letter Cards converted to numerical value
  #  this was made easier since there was no straight of "A 2 3 4 5" in the test set.
  #  if there were some Royal and Straight Flushes and competing Full House hands, more code would've been needed
  set1 = sorted(list(set(rank1)))
  len1 = len(set1)
  if len1 == 2:
    print "full house or 4 of a kind"
    for card in set1:
      if rank1.count(card) == 4:  is4Kind = True
      elif rank1.count(card) ==3:  isFullHouse = True
  elif len1 == 3:
    print "Two Pair or 3 of a kind"
    for card in set1:
      if rank1.count(card) == 3:  is3Kind = True
      elif rank1.count(card) ==2:  isTwoPairs = True  
  elif len1 == 4:
    print "One Pair"
    isOnePair = True
    for card in set1:
      if rank1.count(card) == 2:  OnePairVal = card 
  elif len1 == 5:
  ...
  if len2 == 2:
    print "full house or 4 of a kind"
    for card in set2:
      if rank2.count(card) == 4:  is4Kind2 = True
      elif rank2.count(card) ==3: isFullHouse2 = True
  elif len2 == 3:
    print "Two Pair or 3 of a kind"
    for card in set2:
      if rank2.count(card) == 3:  is3Kind2 = True
      elif rank2.count(card) ==2:  isTwoPairs2 = True 
  elif len2 == 4:
    print "One Pair"
    isOnePair2 = True
    for card in set2:
      if rank2.count(card) == 2: OnePairVal2 = card 
  elif len2 == 5:
    print "High Card Rules"
    isHC2 = True
    HCVal2 = rank2[4]
  ...
  # check for flushes and straights.  straights are easy if you just sum the cards and make sure the last card is
  # 4 more than the first
  ...
  if isOnePair and isOnePair2:
    ...
    elif OnePairVal == OnePairVal2: print "look to next card";continue    # no one pair matches, no need to check further
  ...
  #  some test code that checked gave an initial answer of 360 simply by checking the size of the set of each hand
  #  with the smaller the set (i.e. size of set = 2 means 4 of kind or full house), to set size = 5 means look for 
  #  high card.
  '''
