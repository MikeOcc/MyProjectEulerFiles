    ...
    for m in x:
      ...
      if v>max:
        ...
        if v-m>supermax:    
          ...
          #break
  docont=False
  if sp in rj and superm in rj:
    old1 = notcoprime(sp,l)
    old2 = notcoprime(superm,l)
    if old1> 0 and old1==old2:
      if max > old1:
        rj.remove(superm);rj.remove(sp)
        l.remove((old1,sp,superm))
      else:
        docont = True
    elif old1> 0 and old2>0 and old1!=old2:
      if max > old1:
        l.remove(old1)
